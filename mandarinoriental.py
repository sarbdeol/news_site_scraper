...
if most_recent_href:
    ...
    p_tags_text = generate_news(driver.execute_script(js_code))
    ...
    data = [
        ...
        date,  # Created at (current timestamp)
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),  # Updated at (current timestamp)
        date,  # Added timestamp (current timestamp)
        ...
        "100"  # Placeholder for category_id
    ]

    # Saving the data to CSV
    with open('mandarinoriental_scraped_data.csv', mode='a', newline='', encoding='utf-8') as file:
